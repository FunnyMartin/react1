...
const CountryFlagValidator: React.FC<CountryFlagValidatorProps> = ({ password }) => {
    ...
    const meetsFlagRequirement = password.toUpperCase().includes(selectedCountry);

    return (
        <div>
            <h6>Country Flag Validation</h6>
            {selectedCountry && (
                <div className="d-flex align-items-center">
                    ...
                    />
                    {meetsFlagRequirement ? (
                        <p style={{ margin: 0, color: "green" }}>Password meets the flag requirement</p>
                    ) : (
                        <p style={{ margin: 0, color: "red" }}>Password does not meet the flag requirement</p>
                    )}
                </div>
            )}
        </div>
